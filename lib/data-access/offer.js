...
export async function offerFromListing(item, listingItem, user) {
  await dbConnect();
  try {
    const itemData = await Item.findById(listingItem).exec();

    if (!itemData) {
      throw errorThrower("ItemError", "Item does not exist");
    }

    if (String(itemData.user) !== user) {
      throw errorThrower(
        "ItemUserError",
        "Can't offer item that is not listed by you"
      );
    }
    ...
    const newOffer = new Offer({
      ...
      location: { type: "Point", coordinates: itemData?.location?.coordinates },
    });

    await newOffer.save();

    await Item.findByIdAndUpdate(listingItem, { asOffer: true }).exec();
...
export async function updateOffer(user, offer, values) {
  ...
  try {
    if (
      ...
      const offerData = await Offer.findOne({ user, _id: offer })
        .select("images item")
        .exec();
...
export async function deleteOffer(user, offer) {
  ...
  try {
    const offerData = await Offer.findOne({ user, _id: offer })
      .select("user images item fromListing listingItem")
      .exec();
    ...
    if (offerData?.isFromListing) {
      await Item.findByIdAndUpdate(offerData?.listingItem, { asOffer: false }).exec();
    } else {
...
export async function acceptOffer(offer, user, item, accepted) {
  ...
  try {
    ...
    const updatedOffer = await Offer.findByIdAndUpdate(
      offer,
      { accepted, dateAccepted: new Date() },
      {
        new: true,
      }
    ).exec();
    ...
    if (accepted) {
      const oldAcceptedOffer = await Offer.findOneAndUpdate(
        ...
        { accepted: false, dateAccepted: null },
        { new: true }
      );
    ...
    await Item.findByIdAndUpdate(
      item,
      { ended: accepted, isAcceptedRemoved: false },
      {
        new: true,
      }
    ).exec();
...
export async function getUserOffer(item, user) {
  ...
  try {
    const offerData = await Offer.findOne({ $and: [{ item }, { user }] })
      .populate("user", "firstName lastName fullName image reviewed", User)
      .exec();
