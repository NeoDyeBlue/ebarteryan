...
export async function verify(email, otp) {
  ...
  try {
    ...
    if (!verification) {
      let error = new Error("incorrect or otp expired");
...
export async function sendPasswordResetRequest(email, origin, resend = false) {
  ...
  try {
    ...
    if (!user) {
      throw error("UserError", "Cannot find account");
    }
...
export async function resetPassword(token, newPassword) {
  ...
  try {
    ...
    if (!passwordReset) {
      throw error("TokenError", "Invalid token");
    }
...
export async function changeAccountPassword(user, passwords) {
  ...
  try {
    ...
    if (!isExactPassword) {
      throw error("PasswordError", "Incorrect password");
    }
