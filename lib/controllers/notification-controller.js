...
/**
 * @param {String} type - 'offer' or 'offer-accepted'
 * @param {String} offer - offer ID
 * @param {String} user - user ID that is involved in the notification
 * @param {String} item - item ID
 * @returns {Object} - notification object
 */

export async function offerNotify(type, offer, user, item) {
  await dbConnect();
  try {
    let receiver;
    let notification;
    ...
    if (type == "offer") {
      ...
      if (existingNotification) {
        const updatedNotification = await Notification.findByIdAndUpdate(
          ...
          { $addToSet: { users: qaData.user._id } },
          { new: true }
        )
          .populate(notifPopulateOptions)
          .exec();
...
export async function notify(type, user, item) {
  ...
  try {
    ...
    if (itemData) {
      if (type == "offer" || type == "question" || type == "review") {
